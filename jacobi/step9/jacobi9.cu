...
__global__
void iloop(float* A, float* b, int N, float* y)
{
	...
	{
		X[j] = y[j]; // Since we have syncthreads() for every thread block, this avoids memcpy after kernel execution.
	}
...
int main(int argc, char* argv[])
{
   ...
   readFile(fname, &N, &iter, &A, &b);
	...
	if( cudaMemcpy(d_y, x, N*sizeof(float)  , cudaMemcpyHostToDevice) != cudaSuccess ) printf("CUDA : memory copy error!\n");
	for(k = 0; k < iter; k++)
	{
		iloop<<< numblocks, blocksize, N*sizeof(float) >>>(d_A, d_b, N, d_y); // kernel launch on GPU
