...
__global__
void aes_encrypt(int n, unsigned int *pixel, unsigned int *encrypted, /*unsigned int *rk,*/ unsigned int *smem)
{
   int i;
   ...
   int stride = (gridDim.x * blockDim.x)*4;
	int id = threadIdx.x;
	int bdim = blockDim.x;
   __shared__ unsigned int aes[1324]; // Both the 5 TBOXes and All round key schedules are stored in shared memory.
   __shared__ unsigned int pt[1024];
   __shared__ unsigned int ct[1024];
   unsigned int *tbox0, *tbox1, *tbox2, *tbox3, *tbox4, *rk;
   unsigned int s0, s1, s2, s3, t0, t1, t2, t3; 
 
   for(i = id; i < 1324; i += blockDim.x)
   {
      aes[i] = smem[i];
   }
   __syncthreads();
   ...
   tbox4 = tbox0 + 1024;
	...
	{
		pt[id]          = *((pixel+i));
		pt[id+bdim]     = *((pixel+i+bdim));
		pt[id+(2*bdim)] = *((pixel+i+(2*bdim)));
		pt[id+(3*bdim)] = *((pixel+i+(3*bdim)));
		__syncthreads();
		...
		*((encrypted + i))            = ct[id];
		*((encrypted + i + (bdim)))   = ct[id + (bdim)];
		*((encrypted + i + (2*bdim))) = ct[id + (2*bdim)];
		*((encrypted + i + (3*bdim))) = ct[id + (3*bdim)];
		
		//*((encrypted + (i) + (3*id) + 0))  = s0;	
		//*((encrypted + (i) + (3*id) + 1))  = s1;
		//*((encrypted + (i) + (3*id) + 2))  = s2;
		//*((encrypted + (i) + (3*id) + 3))  = s3;
   }
}
...
int main(int argc, char* argv[])
{
   ...
   if(cudaMallocManaged(&pixel, pixel_size*sizeof(unsigned int)) != cudaSuccess) printf("CUDA: Memory Allocation Error!\n");
   if(cudaMallocManaged(&encrypted, pixel_size*sizeof(unsigned int)) != cudaSuccess) printf("CUDA: Memory Allcation Error!\n");
   //if(cudaMallocManaged(&rk, 44*sizeof(unsigned int)) != cudaSuccess) printf("CUDA: Memory Allcation Error!\n");
   if(cudaMallocManaged(&tbox, 1324*sizeof(unsigned int)) != cudaSuccess) printf("CUDA: Memory Allcation Error!\n");
   ...
   for(i = 0; i < 256; i++)
   {
 		...
 		tbox[i] = (temp2 << 24) + (temp1 << 16) + (temp1 << 8) + (temp3);
 		tbox[256+i] = (temp3 << 24) + (temp2 << 16) + (temp1 << 8) + (temp1);
 		tbox[512+i] = (temp1 << 24) + (temp3 << 16) + (temp2 << 8) + (temp1);
 		tbox[768+i] = (temp1 << 24) + (temp1 << 16) + (temp3 << 8) + (temp2);
 		tbox[1024+i] = (temp1 << 24) + (temp1 << 16) + (temp1 << 8) + (temp1);
   }
   ...
   for(i = 0; i < 10; i++)
   {
 	temp = rk[(4*i)+3];
 	rk[(4*i)+4] = rk[(4*i)+0] ^ 
 		     (tbox[1024+((temp >> 16) & 0xff)] & 0xff000000) ^
 		     (tbox[1024 + ((temp >>  8) & 0xff)] & 0x00ff0000) ^
 		     (tbox[1024 + ((temp      ) & 0xff)] & 0x0000ff00) ^
 		     (tbox[1024 + ((temp >> 24)       )] & 0x000000ff) ^ rcon[i];
   ...
   aes_encrypt<<< numBlocks, blocksize, 3372*sizeof(unsigned int) >>>(pixel_size, pixel, encrypted, /*rk,*/ tbox);
