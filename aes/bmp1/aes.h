...
// Pre-computing keys for all rounds.
void generate_keys(unsigned char* ikey)
{
	int i, r;

	for(i = 0; i < 16; i++) key[i] = *(ikey+i);
	...
	for(r = 1; r < 11; r++)
	{
		key[16*r    ] = sbox[key[16*r - 3]] ^ key[16*r - 16] ^ Rcon[r-1];
		key[16*r + 1] = sbox[key[16*r - 2]] ^ key[16*r - 15];
		key[16*r + 2] = sbox[key[16*r - 1]] ^ key[16*r - 14];
		key[16*r + 3] = sbox[key[16*r - 4]] ^ key[16*r - 13];
		
		for(i = 4; i < 16; i++)
		{
			key[16*r + i] = key[16*r +i-16] ^ key[16*r +i-4];
		}
	   //for(i = 0; i < 16; i++) printf("%02x ", key[16*r+i]);
	   //printf("\n");
	}
...
//void aes_encrypt(unsigned short *istate, unsigned char *ikey)
void aes(unsigned char *state)
{
   ...
   for (round = 0; round < 10; round++)
   {
      for (j = 15; j >= 0; j--)
      {
      	state[j]=state[j] ^ key[16*round+j];
      }
		//for(i = 0; i < 16; i++) printf("%02x ", state[i]);
		//printf("\n");
      // Substitute Bytes
   ...
   for (i = 0; i <16; i++)
   {
      ...
      state[i]=state[i] ^ key[160+i];
   }
		//for(i = 0; i < 16; i++) printf("%02x ", state[i]);
		//printf("\n");
}

void sw_encrypt(unsigned int* ptext, unsigned int* ctext)
{
	u32 temp[4];
	u8 *t;
	...
	*(ctext+2) = GETU32(t);
	t = (u8*) (temp + 3);
	*(ctext+3) = GETU32(t);
	//printf("0x%08x %08x %08x %08x\n", ctext[0], ctext[1], ctext[2], ctext[3]);
}
